...
function TransactionDetailsContent() {
    ...
    const Item = ({ label, value, icon }: { label: string; value: string; icon?: React.ComponentType<{ className?: string; }>; }) => {
    ...
    const Row = ({ items }: { items: { label: string; value: any; icon?: React.ComponentType<{ className?: string; }>; }[]; }) => {
        const filled = [...items];
        ...
        return (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-3 gap-y-3">
                {filled.map((item, i) =>
                    item.label ? <Item key={i} label={item.label} value={item.value} icon={item.icon} /> : <div key={i} className="hidden md:block"></div>
                )}
            </div>
        );
    };
    ...
    return (
        <div className="mx-auto">
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-3 sm:p-4 md:p-6 shadow-sm space-y-4 sm:space-y-5">
                ...
                <Row
                    items={[
                        { label: "Crypto Amount", value: data["Crypto Amount"], icon: Generic },
                ...
                <Row
                    items={[
                        { label: "POS Device Name", value: data["POS Device Name"] },
                ...
                <Row
                    items={[
                        { label: "Payer Email Address", value: data["Payer Email Address"] },
                ...
                <Row
                    items={[
                        { label: "IPN Payment Request ID", value: data["IPN Payment Request ID"] },
